...
export default function BuscarImagenPage() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      setUploadError("Por favor sube un archivo de imagen valido")
      return
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="mb-8 text-center">
          <h1 className="mb-2 text-2xl font-bold text-foreground sm:text-3xl">Buscar por imagen</h1>
          <p className="text-muted-foreground">Sube una foto y encontraremos productos similares en nuestro catalogo</p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-1">
            <div className="rounded-xl border border-border bg-card p-6 shadow-soft">
              ...
              <div className="mb-4">
                {!uploadedImage ? (
                  <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-secondary/30 p-8 text-center transition-colors hover:border-primary/50 hover:bg-secondary/50">
                    <Upload className="mb-3 h-8 w-8 text-muted-foreground" />
                    <p className="mb-1 text-sm font-medium text-foreground">Arrastra tu imagen aqui</p>
                  ...
                  <div className="relative overflow-hidden rounded-lg bg-secondary">
                    <img src={uploadedImage} alt="Uploaded" className="h-40 w-full object-cover" />
                    <button
                      onClick={handleRemoveImage}
                      className="absolute right-2 top-2 cursor-pointer rounded-full bg-destructive/90 p-2 text-white transition-colors hover:bg-destructive"
                    >
                      x
                    </button>
                  </div>
              ...
              <div className="mt-6 space-y-3 rounded-lg bg-secondary/30 p-4">
                ...
                <ul className="space-y-2 text-xs text-muted-foreground">
                  <li>Usa imagenes claras y bien iluminadas</li>
                  <li>Asegurate de que el producto sea el enfoque principal</li>
                  <li>Evita imagenes borrosas o pixeladas</li>
                  <li>Formatos: JPG, PNG, WEBP (max 10MB)</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {searchResults.map((result) => (
                      <Link
                        ...
                        className="group overflow-hidden rounded-xl border border-border bg-card shadow-soft transition-all hover:border-primary/50 hover:shadow-elevated cursor-pointer"
                      >
                        <div className="relative h-40 overflow-hidden bg-secondary">
                          ...
                          />
                          <div className="absolute right-2 top-2 rounded-full bg-primary/90 px-2 py-1 text-xs font-semibold text-primary-foreground">
                  ...
                  <div className="space-y-3">
                    {searchResults.map((result) => (
                      <Link
                        key={result.id}
                        href={`/producto/${result.id}`}
                        className="group flex gap-4 rounded-xl border border-border bg-card p-4 shadow-soft transition-all hover:border-primary/50 hover:shadow-elevated cursor-pointer"
                      >
                        <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-lg bg-secondary">
                          <img
                            ...
                            className="h-full w-full object-cover transition-transform group-hover:scale-110"
                          />
                          <div className="absolute right-1 top-1 rounded-full bg-primary/90 px-1.5 py-0.5 text-xs font-semibold text-primary-foreground">
                            {result.match}%
                          </div>
                        </div>
            ...
            {searchResults.length === 0 && uploadedImage && !isSearching && (
              <div className="rounded-xl border border-border bg-card p-12 text-center shadow-soft">
                ...
                <p className="mb-2 font-medium text-foreground">No se encontraron resultados</p>
                <p className="text-sm text-muted-foreground">Intenta con una imagen diferente o busca usando palabras clave</p>
              </div>
            )}
            ...
            {searchResults.length === 0 && !uploadedImage && !isSearching && (
              <div className="rounded-xl border border-dashed border-border bg-secondary/20 p-12 text-center">
                <ImageIcon className="mx-auto mb-4 h-12 w-12 text-muted-foreground" />
                <p className="text-muted-foreground">Sube una imagen en el panel izquierdo para comenzar la busqueda</p>
              </div>
            )}
          </div>
        </div>
