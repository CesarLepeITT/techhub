...
export async function POST(req: Request) {
  try {
    ...
    if (body.imageDataUrl) {
      imageDescription = await describeImageToText(body.imageDataUrl, normalizedQuery)
      const imageSearchContext = await buildSearchContextFromImage({ normalizedQuery, imageDescription })
      searchQuery = await rewriteSearchQuery(imageSearchContext || normalizedQuery || imageDescription)
    } else {
